...
[<img src="https://github-ads.s3.eu-central-1.amazonaws.com/filament-archivable.jpg?t=1" width="419px" />](https://spatie.be/github-ad-click/filament-archivable)
...
```bash
php artisan vendor:publish --tag="filament-archivable-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="filament-archivable-config"
```
...
```bash
php artisan vendor:publish --tag="filament-archivable-views"
```

## Usage

```php
$filamentArchivable = new Okeonline\FilamentArchivable();
echo $filamentArchivable->echoPhrase('Hello, Okeonline!');
```
